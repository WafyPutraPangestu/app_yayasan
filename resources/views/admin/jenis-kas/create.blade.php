<x-layout>
    <div class="container mx-auto px-4 py-8">
        <h1 class="text-3xl font-bold mb-6">Tambah Jenis Kas Baru</h1>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md  mx-auto" x-data="jenisKasForm()">
            ...
            <form action="{{ route('jenis-kas.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label for="kode_jenis_kas" class="block text-gray-700 text-sm font-bold mb-2">COA:</label>
                ...
                <div>
                    <label for="nama_jenis_kas" class="block text-gray-700 text-sm font-bold mb-2">Nama Jenis
                        Kas:</label>
                    <input type="text" name="nama_jenis_kas" id="nama_jenis_kas" value="{{ old('nama_jenis_kas') }}"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                </div>
                ...
                <div>
                    ...
                    <select name="default_tipe" id="default_tipe"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                        <option value="" disabled selected>-- Pilih Sifat Dasar --</option>
                        <option value="pemasukan" @if (old('default_tipe') == 'pemasukan') selected @endif>Pemasukan</option>
                        <option value="pengeluaran" @if (old('default_tipe') == 'pengeluaran') selected @endif>Pengeluaran
                        </option>
                    </select>
                </div>
                ...
                <div x-show="tipeIuran === 'wajib'" x-transition class="space-y-4">
                    <div>
                        <label for="nominal_wajib" class="block text-gray-700 text-sm font-bold mb-2">Nominal Wajib per
                            Periode (Rp):</label>
                        <input type="number" name="nominal_wajib" id="nominal_wajib" value="{{ old('nominal_wajib') }}"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            min="0">
                    </div>
                    ...
                    <div>
                        <label for="target_lunas" class="block text-gray-700 text-sm font-bold mb-2">Target Lunas Penuh
                            (Rp):</label>
                        <input type="number" name="target_lunas" id="target_lunas" value="{{ old('target_lunas') }}"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            min="0">
    ...
    <script>
        function jenisKasForm() {
            return {
                tipeIuran: '{{ old('tipe_iuran', 'sukarela') }}'
            }
        }
    </script>
</x-layout>
